...
namespace A5Soft.A5App.Application
{
    ...
    public class ExtensionInfo
    {
        ...
        public ExtensionInfo(Guid id, string name, string version)
        {
        ...
        {
            return CompareVersionTo(new ExtensionInfo(Id, Name, versionToCompare));
        }

        /// <summary>
        /// Compares plugin (app extensions) versions similar to IComparable.CompareTo method. 
        /// </summary>
        /// <param name="extension">a plugin (app extension) to compare to</param>
        /// <returns>Returns 1, if the version of the plugin is newer than of the compared one.
        /// Returns -1, if the version of the plugin is older than of the compared one.
        /// Returns 0, if the versions match.</returns>
        /// <exception cref="ArgumentNullException">if <see langword="null"/> extension</exception>
        ...
        /// <exception cref="InvalidOperationException">versions are defined in different formats</exception>
        public int CompareVersionTo(ExtensionInfo extension)
        {
            if (extension.IsNull()) throw new ArgumentNullException(nameof(extension));
            if (extension.Id != Id) throw new InvalidOperationException(
                "Cannot compare versions of different plugins (extensions).");

            if (null != extension._conventionalVersion && null != _conventionalVersion)
            {
                for (int i = 0; i < Math.Min(extension._conventionalVersion.Length, _conventionalVersion.Length); i++)
                {
                    if (extension._conventionalVersion[i] > _conventionalVersion[i]) return -1;
                    if (extension._conventionalVersion[i] < _conventionalVersion[i]) return 1;
                }

                return 0;
            }
            else if (extension._dateVersion != DateTime.MinValue && _dateVersion != DateTime.MinValue)
            {
                if (extension._dateVersion > _dateVersion) return -1;
                else if (extension._dateVersion < _dateVersion) return 1;
                return 0;
            }
            else if (extension._intVersion != int.MinValue && _intVersion != int.MinValue)
            {
                if (extension._intVersion > _intVersion) return -1;
                else if (extension._intVersion < _intVersion) return 1;
                return 0;
            }

            throw new InvalidOperationException(
                $"Cannot compare versions defined in different formats: {Version} vs {extension.Version}.");
        }
