...
namespace A5Soft.A5App.Application.UseCases.Security
{
    ...
    public class UpdateUserUseCase : SaveUseCaseBase<User, IUser>, IUpdateUserUseCase
    {
        ...
        /// <inheritdoc />
        public UpdateUserUseCase(ClaimsIdentity user, IUseCaseAuthorizer authorizer, IClientDataPortal dataPortal,
        ...
        protected override async Task<User> DoSaveAsync(IUser domainDto)
        {
            ...
            if (domainDto.Id.IsSameIdentityAs(User.Sid().ToEntityIdentity<User>()))
            {
                Logger.LogSecurityIssue($"User {User.Name} attempted to update data about self.");
            ...
            if (User.IsGroupAdmin())
            {
                if (null == current.UserGroup ||
                    !User.GroupSid().ToEntityIdentity<UserGroup>().IsSameIdentityAs(current.UserGroup.Id))
                {
                    Logger.LogSecurityIssue($"User {User.Name} attempted to update data of the user " +
                ...
                {
                    Logger.LogSecurityIssue($"User {User.Name} attempted to update data of the admin user " +
                        $"- {current.Email}.");
                ...
                {
                    Logger.LogSecurityIssue($"User {User.Name} tried to set user ({domainDto.Email}) group to null.");
                ...
                {
                    Logger.LogSecurityIssue($"User {User.Name} tried to update user ({domainDto.Email}) group.");
                    throw new ValidationException(Resources.Security_UpdateUserUseCase_Cannot_Update_UserGroup);
                }
                if (domainDto.AdminRole != AdministrativeRole.None)
                {
                    Logger.LogSecurityIssue($"User {User.Name} tried to grant user ({domainDto.Email}) admin privileges.");
