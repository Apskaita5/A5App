...
namespace A5Soft.A5App.Application.UseCases.Security.Users
{
    ...
    public class DeleteUserUseCase : CommandWithParameterUseCaseBase<IDomainEntityIdentity>, IDeleteUserUseCase
    {
        ...
        /// <inheritdoc />
        public DeleteUserUseCase(IAuthenticationStateProvider authenticationStateProvider, 
            ...
            ILogger logger, ICacheProvider cache, IUserRepository repository) 
        ...
        /// <inheritdoc cref="IDeleteUserUseCase.Validate"/>
        ...
        protected override async Task ExecuteAsync(IDomainEntityIdentity parameter)
        {
            ...
            if (parameter.IsSameIdentityAs(identity.Sid().ToIdentity<User>()))
            {
                Logger.LogSecurityIssue($"User {identity.Name} attempted harakiri.");
            ...
            if (identity.IsGroupAdmin())
            {
                ...
                {
                    Logger.LogSecurityIssue($"User {identity.Name} attempted to delete user {userToDelete.Email} " +
                ...
                if (!userToDelete.UserGroup.Id.IsSameIdentityAs(identity.GroupSid().ToIdentity<UserGroup>()))
                {
                    Logger.LogSecurityIssue($"User {identity.Name} attempted to delete user {userToDelete.Email} " +
